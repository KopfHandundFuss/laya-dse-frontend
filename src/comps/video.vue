    ...
    <div v-if="data.src">
      <div v-if="isUpload">
        <video
          ...
          <track v-if="data.sub" kind="subtitles"
          :src="url('sub')" srclang="de" label="Deutsch" default>
        </video>
        <div class="hidden">
          <audio v-if="data.ad"
            :id="id('audiod')"
...
export default {
  ...
  methods: {
    ...
    onMainSeek: function (event) {
      ...
      if (this.data.ad) { this.audiod.currentTime = time }
      if (this.data.adds) this.adds.forEach(vid => { vid.currentTime = time })
    },
    ...
    url: function (type) {
      ...
      if (typeof media === 'string') { /* file in backend */
        return `${aws()}/${this.$route.params.id}/videos/${media}`
      }
