    ...
    <div class="container">
      ...
      <div class="row">
        ...
        <form class="d-flex flex-column align-items-center">
          ...
          <img src="../assets/anmelden.svg" alt="Login" 
          class="d-block m-auto w-50">
          <h1 class="text-center">{{ i18n.title }}</h1>

          <div class="ly-input" :class="{error: errEmail}">
            <input id="email-input" v-model.trim="email"
            :placeholder="i18n.emailPH" type="text" autofocus autocomplete="on">
          </div>

          <div class="ly-input" :class="{error: errPwd}">
            <input v-model.trim="pwd" :placeholder="i18n.pwdPH" type="password"
            autocomplete="on">
          </div>

          <div style="height: 2rem"></div>
          <button id="login-button" type="submit" @click.prevent="submit"
            class="btn btn-lg btn-outline-dark"
            aria-describedby="login-error">
            {{ i18n.submitTxt }} <i class="fas fa-arrow-right"></i>
          </button>
          ...
          <div id="login-error" v-if="submitFailed" :aria-hidden="!submitFailed"
          ...
          <div class="text-center">
            {{ i18n.registerHint1 }}<br>
            <u><a href="#/register">{{ i18n.registerHint2 }}</a></u>
          </div>
        </form>
      </div>
    </div>
...
import lyInput from '@comps/input'
